...
- Loads close-up seduction prompts from
  succubus_closeup_seduction_variations.txt.
...
PROMPTS_FILE = Path(__file__).resolve().parent / "succubus_closeup_seduction_variations.txt"
ANCHOR_IMAGE_PATH = Path("succubus_anchor_01_4x4") / "anchor.png"
OUTPUT_ROOT = Path("./succubus_variations_closeup_seduction")
OUTPUT_STEM_PREFIX = "succubus_closeup_v"
...
def _validate_config() -> None:
    ...
    if not OUTPUT_STEM_PREFIX.strip():
        raise ValueError("OUTPUT_STEM_PREFIX must be non-empty")
...
def load_variation_prompts(path: Path) -> List[VariationEntry]:
    ...
    for match in pattern.finditer(variation_block):
        variation_num = int(match.group(1))
        body = match.group(3).strip()
        if body:
            out.append((variation_num, body.replace("[CHARACTER SHEET]", character_sheet).strip()))
        ...
        for req_i in range(1, req_count + 1):
            ...
            try:
                ...
                for i, image in enumerate(images):
                    filename = f"{OUTPUT_STEM_PREFIX}{variation_num:02d}_{saved + i + 1:02d}.{OUTPUT_FORMAT}"
                    out_path = _non_clobber_path(vdir / filename)
...
async def main() -> None:
    ...
    print(
        f"GPT Image 2 edit run (close-up seduction): variations {START_VARIATION:02d}-{END_VARIATION:02d} "
