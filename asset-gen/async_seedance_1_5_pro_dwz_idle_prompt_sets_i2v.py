...
For every PNG in ./variations_diewithzero (anchor excluded — its idle is
already covered by the original variations run), generate four separate
idle video sets:
...
VARIATIONS_DIR = Path("variations_diewithzero")
OUTPUT_ROOT = Path("video_outputs_seedance_1_5_pro")

PROMPT_SETS = [
    {
        "name": "idle_animations_diewithzero",
        "suffix": "idle",
        "prompt": (
    ...
    {
        "name": "idle_blink_animations_diewithzero",
        "suffix": "idle_blink",
        "prompt": (
            "Create a subtle looping video game idle animation from the provided image. "
            "Use gentle breathing motion, tiny posture shifts, slight hair and cloth movement, "
    ...
    {
        "name": "idle_look_forward_diewithzero",
    ...
    {
        "name": "idle_body_trace_diewithzero",
...
async def main() -> None:
    ...
    if total_failures:
        ...
        print("Done. All prompt sets completed successfully.")
