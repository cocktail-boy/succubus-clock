...
idle animation with natural eye blinks by using the same PNG as both the start
and end frame.
"""
...
OUTPUT_DIR = Path("video_outputs_seedance_1_5_pro") / "idle_blink_animations"
...
async def generate_idle_video(
    *,
    ...
    print(f"[{label}] submitting {IDLE_DURATION}s blink idle animation...")
...
async def process_image(name: str, image_path: Path, *, sem: asyncio.Semaphore) -> None:
    async with sem:
        out_path = OUTPUT_DIR / f"{name}_idle_blink_{IDLE_DURATION}s.mp4"
        if out_path.exists():
...
async def main() -> None:
    ...
    all_jobs = image_jobs()
    if not all_jobs:
        print("No blink idle animation image jobs found.")
        return
    ...
    if not jobs:
        print("No blink idle animation jobs to process after SKIP_FIRST / limit.")
        return

    print(f"Processing {len(jobs)} blink idle animation image job(s).")
    ...
    if failures:
        print(f"Done with {failures}/{len(jobs)} blink idle animation failure(s).")
    else:
        print(f"Done. Saved {len(jobs)} blink idle animation video(s) to {OUTPUT_DIR.resolve()}")
